-- Comprehensive fix for Supabase setup issues
-- This migration ensures everything is configured correctly

-- 1. Fix countries RLS to allow public read access
DROP POLICY IF EXISTS "Countries are viewable by everyone" ON public.countries;
CREATE POLICY "Countries are viewable by everyone"
  ON public.countries FOR SELECT
  USING (true);

-- 2. Ensure countries table has all countries (refresh with full list)
...
-- 3. Ensure profiles RLS allows public read for username checking
DROP POLICY IF EXISTS "Profiles are readable publicly" ON public.profiles;
CREATE POLICY "Profiles are readable publicly"
  ON public.profiles FOR SELECT
  USING (true);

-- 4. Ensure anonymous auth is enabled (note: this requires Supabase dashboard configuration)
